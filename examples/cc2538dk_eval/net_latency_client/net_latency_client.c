...
void send_data(void) {
	...
	EVAL_LED2_TOGGLE;
	if (ret<0) {
		PRINTF("udp_socket_send(...) failed: %d\n", ret);
	} else {
		PRINTF("udp_socket_send(...) sent %d bytes\n", ret);
	}
...
PROCESS_THREAD(udp_client_process, ev, data) {
	...
	PROCESS_BEGIN();
	PRINTF("net_latency_client process started\n");
	...
	while(1) {
		...
		switch (ev) {
			...
			case PROCESS_EVENT_USER1: {
				etimer_set(&et, EXPERIMENT_DELAY*CLOCK_SECOND);
				break;
			}
			default: {
