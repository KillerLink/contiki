	...
	uint16_t datalen)
{
	...
	if (bytesReceived >= DATA_LENGTH) {
		EVAL_LED1_TOGGLE;
		PRINTF("bytesReceived = %d, errorCount = %d\n", bytesReceived, errorCount);
		bytesReceived = 0;
	}
...
/*
	if (udp_socket_connect(&socket, NULL, UIP_HTONS(SERVER_PORT)) != 1) {
		PRINTF("udp_socket_connect failed!");
		panic();
	}
*/
}
...
void ip_setup (void) {
	...
	if (uip_ds6_maddr_add(&ipv6mserver) == NULL) {
		PRINTF("uip_ds6_maddr_add() failed!");
		panic();
	}
	//add a private network address
	uip_ds6_addr_add(&ipv6server, 0, ADDR_MANUAL);
}
...
PROCESS_THREAD(udp_server_process, ev, data) {
	...
	PROCESS_BEGIN();
	PRINTF("net_latency_server process started\n");
